...
def get_alpha(rot,is_alphay = False):
  # output: (B, 8) [bin1_cls[0], bin1_cls[1], bin1_sin, bin1_cos, 
  #                 bin2_cls[0], bin2_cls[1], bin2_sin, bin2_cos]
  # return rot[:, 0]
  mul = int(is_alphay)*8
  idx = rot[:, mul+1] > rot[:, mul+5]
  alpha1 = np.arctan2(rot[:, mul+2], rot[:, mul+3])
  alpha2 = np.arctan2(rot[:, mul+6], rot[:, mul+7])
  return ((alpha1 * idx + alpha2 * (1 - idx)) + (2*np.pi))%(2*np.pi)
